...
SELECT 
   COUNT(*),
   SUM(CASE WHEN discount_pct = '' then 1 else 0 end) AS MISSING_DISCOUNT,
   ROUND(100.0*SUM(CASE WHEN discount_pct IS NULL OR discount_pct = '' then 1 else 0 end)/COUNT(*),2) AS PERCENT
FROM sales_cleaned;
